...
std::unordered_map<std::string, std::weak_ptr<Shader::Pimpl::ShaderProgram>> Shader::Pimpl::_sharedShaders[Shader::Pimpl::ShaderTypesCount];
...
Shader::Pimpl::Pimpl(Shader &that,
    ...
    for(int shaderType = 0; shaderType < ShaderTypesCount; ++shaderType) {
        if(shaderPath[shaderType] && !shaderPath[shaderType]->empty()) {
            _shaders[shaderType] = _sharedShaders[shaderType][*shaderPath[shaderType] + prepDefines].lock();
...
void Shader::Pimpl::setTexture(char const *name, Texture const &tex) {
    ...
    if(it != _samplers.end()) {
        ...
        // glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, Parametres::anisotropy());
    } else {
...
void Shader::Pimpl::pushBuffer(std::string const &uniformBlockName, std::shared_ptr<UniformBuffer> const &buffer) {
    buffer->bind();
    ...
    if(_current != &_that) {
        ...
        for(auto i = _buffers.begin() + 1; i != _buffers.end(); ++i) {
            ...
            if(std::get<2>(*i)->bindingPoint() == -1) {
                ...
                std::get<2>(*i)
                ->setBindingPoint(static_cast<GLuint>(std::distance(_uniformBlockBindings.begin(), it)));
            } else {
            ...
            if(std::get<0>(*i) != GL_INVALID_INDEX && !std::get<1>(*i)) {
                ...
                glBindBufferBase(GL_UNIFORM_BUFFER, std::get<2>(*i)->bindingPoint(), std::get<2>(*i)->identifier());
                std::get<1>(*i) = true;
            }
        }
    }
...
void Shader::Pimpl::getSamplers(std::string const &shaderSource,
    ...
    if(shaderType == GL_FRAGMENT_SHADER) {
        ...
        std::string pattern = "\nuniform sampler";
        while((it = std::search(it, shaderSource.end(), pattern.begin(), pattern.end())) != shaderSource.end()) {
            Sampler sampler;
