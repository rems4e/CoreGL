...
Framebuffer::Framebuffer(ivec2 const &size, GLint samples, bool stencil)
        : RenderTarget(size), _colorBuffers(), _samples(samples), _stencil(stencil) {
    GLint maxSize = 0;
...
void Framebuffer::resize(ivec2 const size, GLsizei samples) {
    ...
    if(_samples > 1)
        glRenderbufferStorageMultisample(GL_RENDERBUFFER, _samples, _stencil ? GL_DEPTH24_STENCIL8 : GL_DEPTH_COMPONENT24, size.x, size.y);
    else
        glRenderbufferStorage(GL_RENDERBUFFER, _stencil ? GL_DEPTH24_STENCIL8 : GL_DEPTH_COMPONENT24, size.x, size.y);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER,
                           ...
                           _colorBuffers[0].tex(),
                           0);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, _stencil ? GL_DEPTH_STENCIL_ATTACHMENT : GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, _renderBuffer);
