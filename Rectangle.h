...
//  Ce fichier et son contenu sont librement distribuables, modifiables et utilisables pour toute œuvre non commerciale,
...
struct Box {
    Box() = default;
    Box(vec3 const &o, vec3 const &dim) : origin(o), size(dim) {}
...
struct Rectangle {
    ...
    Rectangle(coordinate_t const &g, coordinate_t const &h, dimension_t const &la, dimension_t const &ha)
            : left(g), top(h), width(la), height(ha) {}
    Rectangle(vec2 const &origine, vec2 const &taille)
            : left(origine.x), top(origine.y), width(taille.x), height(taille.y) {}
    Rectangle(Rectangle const &r, int) : left(0), top(0), width(r.width), height(r.height) {}
    ...
    bool operator<(Rectangle const &r) const { // Inclusion stricte dans r
        return left >= r.left && top >= r.top && left + width <= r.left + r.width && top + height <= r.top + r.height && *this != r;
    }
    ...
    bool superposition(Rectangle const &r2) const {
        return ((left >= r2.left && left < r2.left + r2.width) || (left < r2.left && left + width > r2.left)) &&
