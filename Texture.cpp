...
template <>
struct std::hash<std::pair<GLenum, Texture::Sampling>> {
    size_t operator()(std::pair<GLenum, Texture::Sampling> const &tex) const noexcept;
};
...
struct Texture::TextureBase final {
    ...
    inline TextureBase(GLenum target, Sampling sampling) : _target(target), _sampling(sampling) {
...
size_t std::hash<std::pair<GLenum, Texture::Sampling>>::operator()(std::pair<GLenum, Texture::Sampling> const &tex) const noexcept {
    static std::hash<uint64_t> hash;
    return hash(tex.second + (tex.first << Texture::NumSampling));
}
...
void TextureBase::load(GLubyte const *img, ivec2 const &size, int depth, GLsizei samples, bool bgr, bool depthTexture) {
    ...
    if(samples > 1) {
        ...
        glTexImage2DMultisample(
            GL_TEXTURE_2D_MULTISAMPLE, samples, GL_RGBA8, static_cast<GLsizei>(size.x), static_cast<GLsizei>(size.y), GL_TRUE);
        _depth = 4;
    } else {
        ...
        if(depthTexture) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, static_cast<GLsizei>(size.x), static_cast<GLsizei>(size.y), 0, format, GL_UNSIGNED_BYTE, img);
...
void Texture::setAnisotropy(GLfloat anisotropy) {
    ...
    glSamplerParameterf(TextureBase::getSampler(GL_TEXTURE_2D, StandardSampling), GL_TEXTURE_MAX_ANISOTROPY_EXT, anisotropy);
}
