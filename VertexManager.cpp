...
void VertexManager::pushVertex(vec2 const &pos, vec2 const &posTex, vec4 const &couleur) {
    _vbo[_currentBuffer]->setData(vec3(pos.x, pos.y, 0), _nbSommets * (3 + 2 + 4) * sizeof(GLfloat), 3 * sizeof(GLfloat));
...
void VertexManager::pushLine(vec3 const &depart, vec3 const &arrivee, vec4 const &c) {
    ...
    glLineWidth(3);
    VertexManager::drawVertices(
        GL_LINE, 2, *_lineBuffer, 0, VertexManager::npos, 3 * sizeof(GLfloat), VertexManager::npos, (3 + 4) * sizeof(GLfloat));
}
...
void VertexManager::flush() {
    if(_nbSommets) {
        ...
        _vao->vertexAttribPointer(TEX_COORDS_LOCATION, 2, GL_FLOAT, GL_FALSE, (3 + 2 + 4) * sizeof(GLfloat), 3 * sizeof(GLfloat));
        _vao->vertexAttribPointer(COLOR_LOCATION, 4, GL_FLOAT, GL_TRUE, (3 + 2 + 4) * sizeof(GLfloat), (3 + 2) * sizeof(GLfloat));
        _vao->endEditing();
...
void VertexManager::pushTex(Texture const &tex, vec2 const &position, Rectangle scissors, bool upsideDown) {
    ...
    VertexManager::pushVertex(VertexManager::pixelToScreen(vec2(vert.left + vert.width, vert.top + vert.height)),
                              vec2((scissors.left + scissors.width) / dim.x, (scissors.top + scissors.height) / dim.y),
                              c);
    VertexManager::pushVertex(VertexManager::pixelToScreen(vec2(vert.left + vert.width, vert.top)),
                              vec2((scissors.left + scissors.width) / dim.x, scissors.top / dim.y),
                              c);
